...
export const useTaskStore = create<TaskStore>()(persist(
    (set) => ({
            ...
            set((state) => ({
                journals: state.journals.map((journal) =>
                    journal.id === state.activeJournalId
                        ? { ...journal, tasks: [...journal.tasks, { id: uuidv4(), description, subTasks: [], priority: priority, completed: false }] }
                        : journal
                )
            }));
        },
        ...
        editTask: (id: string, description: string, priority: TaskPriority) => {
            if (!description.trim())
                return;
            ...
            set((state) => ({
                journals: state.journals.map((journal) =>
                    journal.id === state.activeJournalId
                        ? { ...journal, tasks: journal.tasks.map((task) => task.id === id ? { ...task, description, priority } : task) }
                        : journal
                )
            }));
        },
